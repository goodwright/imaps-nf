...
xlinks_files = sorted(
    ["xlinks/" + f for f in os.listdir("xlinks") if f.endswith(".bed.gz")]
)
...
for xlinks_file in xlinks_files:
    ...
    exp = re.sub(".bed.gz", "", os.path.basename(xlinks_file))
...
def get_peaks_metrics(peakcaller):
    ...
    peak_files = sorted(
        [peakcaller + "/" + f for f in os.listdir(peakcaller) if f.endswith(".bed.gz")]
    )
    ...
    for peak_file in peak_files:
        ...
        if peakcaller == "icount":
            exp = re.sub(".peaks.bed.gz", "", os.path.basename(peak_file))
        elif peakcaller == "paraclu":
            exp = re.sub(".peaks.bed.gz", "", os.path.basename(peak_file))
        elif peakcaller == "clippy":
            exp = re.sub(
                "_rollmean.+_stdev.+_minGeneCount.+.bed.gz",
                "",
                os.path.basename(peak_file),
            )

        xlinks_df = read_bed("xlinks/" + exp + ".bed.gz")
